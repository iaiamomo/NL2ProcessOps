...
TEMPLATE_FS = """You are a very proficient assistant expert in Business Process Management tasks. You are able in taking a natural language process description and extracting the underline start event, end event, task, exclusive gateway and parallel gateway from the process description. Your goal is to output a list of the extracted tasks.
...
TEMPLATE_NFS = """You are a very proficient assistant expert in Business Process Management tasks. You are able in taking a natural language process description and extracting the underline start event, end event, task, exclusive gateway and parallel gateway from the process description. Your goal is to output a list of the extracted tasks.
...
class TaskLLM():
    ...
    def __init__(self, model, openai_key, temperature=0.0, few_shot=False):
        ...
        if few_shot:
            self.prompt = PromptTemplate.from_template(TEMPLATE_FS)
        else:
            self.prompt = PromptTemplate.from_template(TEMPLATE_NFS)
...
class ProcessLLM:
    ...
    def __init__(self, model="gpt-3.5-turbo", openai_key=None, temperature=0.0, few_shot=False):
        self.tasks_llm = TaskLLM(model, openai_key, temperature=temperature, few_shot=few_shot)
        ...
        for fs in few_shot:
            ...
            if fs:
                if model == "gpt-3.5-turbo-16k":
                    res_df.to_csv('eval_task_fs_gpt35.csv', sep=',', index=False)
                else:
                    res_df.to_csv('eval_task_fs_gpt4.csv', sep=',', index=False)
            else:
                if model == "gpt-3.5-turbo-16k":
                    res_df.to_csv('eval_task_nfs_gpt35.csv', sep=',', index=False)
                else:
                    res_df.to_csv('eval_task_nfs_gpt4.csv', sep=',', index=False)